...
def putdata(testdata, results)
  100.times {
    rowid = UUIDTools::UUID.random_create.to_s

    x = Random.rand(1000)
     
    testdata.insert({ "rowid" => rowid, "x" => x })
    results.insert ({ "rowid" => rowid, "sq" => x*x, "cube" => x*x*x })
  }
  puts "Data inserted... "
end

def verifydata(testdata, results)
  puts "Verifying..."
  
  testdata.find.each { |row| 
    puts row.inspect
    rowid = row["rowid"]
    x = row[:x].to_i
  
    result_row = results.find_one({ :rowid => rowid })
    puts result_row.inspect

    sq = x*x
    cube = x*x*x

    if (result_row[:sq].to_i != sq || result_row[:cube].to_i != cube)
      puts "Row verification failed for row: #{row.inspect}"
    end
  }
